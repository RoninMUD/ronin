...
int mobrescue(CHAR *mob, CHAR *ch, int cmd, char *arg) {
  if (cmd == MSG_MOBACT) {
    if (!mob || (CHAR_REAL_ROOM(mob) == NOWHERE) || (GET_POS(mob) == POSITION_FIGHTING) || !AWAKE(mob)) return FALSE;

    for (CHAR *temp_ch = ROOM_PEOPLE(CHAR_REAL_ROOM(mob)), *next_ch = NULL; temp_ch; temp_ch = next_ch) {
      next_ch = temp_ch->next_in_room;

      if (GET_OPPONENT(temp_ch) && (GET_OPPONENT(temp_ch) != mob) && IS_MORTAL(temp_ch)) {
