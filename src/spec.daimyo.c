    ...
    if (tank) {
      switch ( number(0,4) ) {
        case 1:
          ...
          if ( chance(33) ||
              IS_SET(ROOM_FLAGS(kick_room), NO_MAGIC) ||
              IS_SET(ROOM_FLAGS(kick_room), DEATH) ||
              IS_SET(ROOM_FLAGS(kick_room), PRIVATE) ||
              IS_SET(ROOM_FLAGS(kick_room), NO_BEAM)) {
            kick_room = real_room(DM_PORTAL_INSIDE);
          }
