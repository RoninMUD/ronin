...
int board(OBJ *obj, CHAR *ch, int cmd, char *arg) {
  if (!ch || !GET_DESCRIPTOR(ch)) return FALSE;

  /* Check if board is already loaded. */
  int found = FALSE;

  for (board_t *board = board_list; board && !found; board = board->next) {
    if (board->vnumber == V_OBJ(obj)) {
      found = TRUE;
    }
  }

  /* If the board was not found, initialize it. */
  if (!found) {
...
board_t *load_board(OBJ *obj) {
  board_t *board = NULL;
  FILE *board_fl = NULL;

  /* Check if the board is already loaded. If it is, return NULL. */
  for (board_t *temp_board = board_list; temp_board; temp_board = temp_board->next) {
    if (temp_board->vnumber == V_OBJ(obj)) {
      return NULL;
    }
  }

  /* Allocate the board. */
  CREATE(board, board_t, 1);

  /* Setup the board using values from the board object and prepend the board to the head of the board list. */
  board->vnumber = V_OBJ(obj);
  board->min_read = OBJ_VALUE(obj, 0);
  board->min_write = OBJ_VALUE(obj, 1);
  board->min_remove = OBJ_VALUE(obj, 2);
  board->name = str_dup(OBJ_NAME(obj));
  board->short_desc = str_dup(OBJ_SHORT(obj));
  board->next = board_list;
  board_list = board;

  if ((board_fl = fopen(save_file(board), "r") == NULL)) {
  ...
  fread(&board->msg_num, sizeof(board->msg_num), 1, board_fl);

  /* Validate the number of messages and that the board has more data to read. */
  if (board->msg_num < 1 || board->msg_num > MAX_MSGS || feof(board_fl)) {
    log_f("Board: Message file corrupt or nonexistent.");
  ...
  for (int i = 0; i < board->msg_num; i++) {
    int len = 0;

    fread(&len, sizeof(int), 1, board_fl);

    CREATE(board->heading[i], char, (size_t)len + 1);
    ...
    fread(board->heading[i], sizeof(char), len, board_fl);
    fread(&len, sizeof(int), 1, board_fl);
    board->msgs[i] = (char *)malloc(len + 1);
    ...
    fread(board->msgs[i], sizeof(char), len, board_fl);
  }
...
void reset_board(board_t *board) {
  for (int i = 0; i < MAX_MSGS; i++) {
    DESTROY(board->heading[i]);
    DESTROY(board->msgs[i]);
  }
