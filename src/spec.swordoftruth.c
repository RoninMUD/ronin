...
int sot_emma(CHAR *emma, CHAR *ch, int cmd, char *arg) {
  ...
  if(cmd == MSG_MOBACT && emma->specials.fighting && chance(20))
  {
    ...
    if (vict)
    {
      ...
      damage(emma,vict,400,TYPE_UNDEFINED,DAM_SKILL);
    }
  }
  return FALSE;
}
...
int sot_gar(CHAR *gar, CHAR *ch, int cmd, char *arg) {
  ...
  if(cmd == MSG_MOBACT && gar->specials.fighting && chance(20))
  {
    ...
    if (vict)
    {
      ...
      if(V_MOB(gar)==GAR_LONG_TAIL) damage(gar,vict,200,TYPE_UNDEFINED,DAM_SKILL);
      else if(V_MOB(gar)==GAR_SHORT_TAIL) damage(gar,vict,100,TYPE_UNDEFINED,DAM_SKILL);
      //heal gar for 5%
