...
#define ROOM_CHAOTIC(rm) (CHAOSMODE || ((rm != -1) ? IS_SET(world[rm].room_flags, CHAOTIC) : FALSE))
#define ROOM_SAFE(rm) (!CHAOSMODE && ((rm != -1) ? IS_SET(world[rm].room_flags, SAFE) : FALSE))
#define ROOM_ARENA(rm) ((rm != -1) && IS_SET(world[CHAR_REAL_ROOM(ch)].room_flags, ARENA) ? TRUE : FALSE)
