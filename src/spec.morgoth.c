                ...
                switch (number(0, 3))
                {
                    case 0: /* Life Siphon */
                        ...
                        if (vict)
                        {
                            ...
                            act("$n calmly touches you. \"You don't need this lifeforce, do you?\"", 0, mob, 0, vict, TO_VICT);

                            GET_HIT(vict) = GET_HIT(vict) * 0.90;
                            GET_HIT(mob) = MIN(GET_MAX_HIT(mob),GET_HIT(mob) + (GET_MAX_HIT(mob) / 20));

                            act("A sickly green glow flows from $N into $n.", 0, mob, 0, vict, TO_ROOM);
                        }
                        break;
                    ...
                    case 1: /* Soul Rend */
                        ...
                        if (vict)
                        {
                            act("$n extends his palm toward $N, ripping at the very fabric of $S soul.", 0, mob, 0, vict, TO_NOTVICT);
                            act("Agony tears through you as $n rends at your soul!", 0, mob, 0, vict, TO_VICT);

                            damage(mob, vict,(GET_HIT(vict) / 10) + number(100, 150),TYPE_UNDEFINED, DAM_MAGICAL);
                    ...
                    case 2: /* Corpse Eruption */
                        act("$n slams his staff into the ground as corpses claw upward!", 0, mob, 0, 0, TO_ROOM);
                        ...
                        for (vict = ROOM_PEOPLE(CHAR_REAL_ROOM(mob)); vict; vict = CHAR_NEXT_IN_ROOM(vict))
                        {
                            ...
                            damage(mob, vict, number(300, 500),	TYPE_UNDEFINED, DAM_PHYSICAL);
                    ...
                    case 3: /* Mark of the Black Sepulcher */
                        act("$n carves a black sigil into the air, sealing a pact with the grave.", 0, mob, 0, 0, TO_ROOM);
                        ...
                        for (vict = world[CHAR_REAL_ROOM(mob)].people; vict; vict = vict->next_in_room)
                        {
                            ...
                            if(chance(50)){
                                act("A deathly chill coils around you as the necromancer's curse takes hold!", 	0, mob, 0, vict, TO_VICT);
                                act("$N shudders as black veins crawl across $S skin.", 0, mob, 0, vict, TO_ROOM);
                                affect_apply(vict, SPELL_BLINDNESS, 0, APPLY_HITROLL, -4, AFF_BLIND, 0);
                                affect_apply(vict, SPELL_BLINDNESS, 0, APPLY_STR, -4, AFF_BLIND, 0);
                                affect_apply(vict, SPELL_BLINDNESS, 0, APPLY_DAMROLL, -4, AFF_BLIND, 0);
                                affect_apply(vict, SPELL_BLINDNESS, 0, APPLY_ARMOR, 20, AFF_BLIND, 0);
                            }

                            /* Small chance to root/paralyze briefly */
                            if (chance(25))
                            {
                                act("Your muscles seize as grave-cold locks your body in place!", 0, mob, 0, vict, TO_VICT);
