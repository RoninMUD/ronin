...
int get_skill_random_number(int chosen_skill)
{
    ...
    { // #define APPLY_MANA_REGEN       53
        random_skill_number = number(0, 6);
    }
    else if (chosen_skill == APPLY_HP_REGEN)
    { // #define APPLY_HP_REGEN         52
        random_skill_number = number(0, 20);
    }
    else if (chosen_skill == APPLY_HIT)
    { // #define APPLY_HIT              13
        random_skill_number = number(0, 150);
    }
        ...
        if (!OBJ_CONTAINS(sepulcher))
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (chance(10))
                {
                    ...
                    // 75% chance to set 1, 30% chance to set 2, and then 10% chance to set 3
...
int gravepiercer_relic(OBJ *obj, CHAR *ch, int cmd, char *arg)
{
    ...
    case MSG_TICK:
        ...
        if (zone == world[real_room(UNDEAD_CITY_ZONE_CHECK)].zone)
        {
