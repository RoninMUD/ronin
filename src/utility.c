...
/*Zone Character Tracker Functions.

These functions will track characters that enter zones and enter a timestamp.

It can be used to ensure characters cannot re-enter areas within a certain timeframe.
 ...
 * ============================================================ */
time_t zct_get_current_timestamp(void) {
    return time(NULL);
}
 ...
 * Ensure file exists
 * ============================================================ */
bool zct_ensure_progress_file(void) {
    FILE *fp = fopen(PROGRESS_TRACKER_FILE, "a+");
    if (!fp) {
        perror("ERROR: Could not create progress tracker file");
        return FALSE;
    }
    fclose(fp);
    return TRUE;
...
/* ============================================================
 * Check if timestamp is expired based on hour difference
 * ============================================================ */
bool zct_is_timestamp_expired(time_t ts, int hours) {
    time_t now = zct_get_current_timestamp();
    double diff_sec = difftime(now, ts);
    double limit_sec = (double)hours * 3600.0;

    return diff_sec > limit_sec;
}

/* ============================================================
 * Write a single character progress entry (only if missing)
 * ============================================================ */
bool zct_record_single_progress(const char *name, int zone) {
    FILE *fp_r, *fp_w;
    char buffer[256];
    char file_name[128], file_zone[32], file_ts[64];
    bool exists = FALSE;

    if (!zct_ensure_progress_file())
        return FALSE;

    fp_r = fopen(PROGRESS_TRACKER_FILE, "r");
    if (!fp_r) {
        perror("ERROR: Cannot open tracker file for reading");
        return FALSE;
    }

    /* Check if entry already exists */
    while (fgets(buffer, sizeof(buffer), fp_r)) {
        if (sscanf(buffer, "%127s %31s %63s", file_name, file_zone, file_ts) == 3) {
            if (!strcmp(file_name, name) && atoi(file_zone) == zone) {
                exists = TRUE;
                break;
            }
        }
    ...
    if (exists)
        return TRUE; /* already recorded once */

    /* Append new entry */
    fp_w = fopen(PROGRESS_TRACKER_FILE, "a");
    if (!fp_w) {
        perror("ERROR: Cannot append to tracker file");
        return FALSE;
    }

    fprintf(fp_w, "%s %d %ld\n", name, zone, zct_get_current_timestamp());
    fclose(fp_w);
...
/* ============================================================
 * Record a group’s first-entry timestamp
 * ============================================================ */
bool zct_record_group_progress(struct char_data *leader, int zone) {
    ...
    zct_record_single_progress(GET_NAME(leader), zone);
    ...
    for (f = leader->followers; f; f = f->next) {
        ...
        if (ch && !IS_NPC(ch)){
			if(IS_AFFECTED(f->follower, AFF_GROUP)){
				zct_record_single_progress(GET_NAME(ch), zone);
			}
		}
    }
...
/* ============================================================
 * Check if a specific character can enter the zone
 * ============================================================ */
bool zct_check_character_progress(const char *name, int zone, int minutes_diff) {
    FILE *fp;
    char buffer[256];
    char file_name[128], file_zone[32];
    time_t file_ts;

    if (!zct_ensure_progress_file())
        return FALSE;

    fp = fopen(PROGRESS_TRACKER_FILE, "r");
    if (!fp) {
        perror("ERROR: Cannot open tracker file for zone check");
        return FALSE;
    }

    while (fgets(buffer, sizeof(buffer), fp)) {
        if (sscanf(buffer, "%127s %31s %ld", file_name, file_zone, &file_ts) == 3) {
            if (!strcmp(file_name, name) && atoi(file_zone) == zone) {

                /* Check expiration (now minutes-based) */
                if (zct_is_timestamp_expired(file_ts, minutes_diff)) {
                    fclose(fp);
...
/* ============================================================
 * Remove entries older than hours_diff
 * ============================================================ */
bool zct_cleanup_old_progress(int zone, int minutes_diff) {
    FILE *fp_r, *fp_w;
    char buffer[256];
    char name[128], file_zone[32];
    time_t ts;
    ...
    fp_r = fopen(PROGRESS_TRACKER_FILE, "r");
    if (!fp_r) {
        perror("ERROR: Cannot open tracker file for cleanup");
        return FALSE;
    }

    fp_w = fopen("src/util/progress_tracker.tmp", "w");
    if (!fp_w) {
        perror("ERROR: Cannot create temp cleanup file");
        fclose(fp_r);
        return FALSE;
    }
    ...
    while (fgets(buffer, sizeof(buffer), fp_r)) {
        ...
        if (sscanf(buffer, "%127s %31s %ld", name, file_zone, &ts) == 3) {

            int entry_zone = atoi(file_zone);
            bool expired = zct_is_timestamp_expired(ts, minutes_diff);

            if (entry_zone == zone) {
                /* Only clean this zone */
                if (!expired) {
                    fprintf(fp_w, "%s %s %ld\n", name, file_zone, ts);
                }
            } else {
                /* Always keep entries for other zones */
                fprintf(fp_w, "%s %s %ld\n", name, file_zone, ts);
            }
        }
    }
    ...
    if (remove(PROGRESS_TRACKER_FILE) != 0) {
        perror("ERROR: cleanup remove failed");
        return FALSE;
    }
    if (rename("src/util/progress_tracker.tmp", PROGRESS_TRACKER_FILE) != 0) {
        perror("ERROR: cleanup rename failed");
        return FALSE;
    }
