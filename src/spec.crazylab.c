...
#define NEPEDESTAL 14434
*/
if(count_mortals_real_room(real_room(SWPEDESTAL), TRUE) < 1 ||
count_mortals_real_room(real_room(SEPEDESTAL), TRUE) < 1 ||
count_mortals_real_room(real_room(NWPEDESTAL), TRUE) < 1 ||
count_mortals_real_room(real_room(NEPEDESTAL), TRUE) < 1
) {return FALSE;   }
else{ return TRUE;}

}
void cl_LightWallLink(int room, CHAR *ch, int cmd, char *arg)
{
...
/* Don't waste any more CPU time if no one is in the room. */
  ...
  if(cmd == MSG_MOBACT && pinksheep->specials.fighting && chance(45))
  {
    ...
    if(cmd == MSG_MOBACT && chance(15)) {
      do_say(bag,"BAhhhhh BahAhhhhh bah.", CMD_SAY);
    }
...
/* Don't waste any more CPU time if no one is in the zone. */
if(count_mortals_zone(explodeszombie, TRUE) < 1) return FALSE;
...
if(count_mortals_room(explodeszombie, TRUE) < 1) return FALSE;
  ...
  if(cmd == MSG_MOBACT && explodeszombie->specials.fighting && chance(10)) {
    ...
    if(chance(30)) {
      vict = get_random_victim_fighting(bag);
      if (vict) {
        do_say(bag,"Here, have an explosion. I love them.  I hope you do too.", CMD_SAY);

        act("$n glows a little and a small explsion forms around $N.",0,bag,0,vict,TO_NOTVICT);
        act("$n glows a little and an explosion forms around you.",0,bag,0,vict,TO_VICT);
        sprintf(buf, "%s is stuck by %s!", GET_NAME(vict), "a small explosion.");
        act(buf, FALSE, bag, NULL, vict, TO_NOTVICT);
        sprintf(buf, "You are stuck by %s!", "a small explosion.");
        act(buf, FALSE, bag, NULL, vict, TO_VICT);
        damage(bag,vict,800,TYPE_UNDEFINED,DAM_PHYSICAL);
      }
    }
  }
...
int cl_biglad(CHAR *biglad, CHAR *ch, int cmd, char *arg) {
  ...
  /* Don't waste any more CPU time if no one is in the zone. */
if(count_mortals_zone(biglad, TRUE) < 1) return FALSE;
...
/* Don't waste any more CPU time if no one is in the zone. */
if(count_mortals_zone(biglargehuge, TRUE) < 1) return FALSE;
...
if(count_mortals_room(biglargehuge, TRUE) < 1) return FALSE;
  ...
  switch (cmd)
  {
    ...
    // Damage Random Victim
      ...
      break;
    case CMD_BASH || CMD_PUMMEL || CMD_SMITE :/* blocks pummel and smite.*/
  ...
  case CMD_LUNGE || CMD_AMBUSH || CMD_BACKSTAB:
...
/* Don't waste any more CPU time if no one is in the zone. */
if(count_mortals_zone(neolathotep, TRUE) < 1) return FALSE;
  ...
  if(cmd == MSG_MOBACT && rosebush->specials.fighting && chance(38))
  {
    ...
    if(chance(65)) {
      ...
      for(vict = world[CHAR_REAL_ROOM(rosebush)].people; vict; vict = next_vict)
      {
        next_vict = vict->next_in_room;
        if(!(vict) || IS_NPC(vict) || !(IS_MORTAL(vict))) continue;
        act("$n launches a poison covered thorn at you and it embeds in your skin,",0,neolathotep,0,vict,TO_VICT);
...
if(count_mortals_room(catrabbit, TRUE) < 1) return FALSE;
  ...
  if(cmd == MSG_MOBACT && catrabbit->specials.fighting && chance(30))
  {
    int turns=0;
    turns++;
  ...
  if(cmd == MSG_MOBACT && cockneysparrow->specials.fighting && chance(70))
  {
    ...
    if(chance(30)) {
      ...
      for(vict = world[CHAR_REAL_ROOM(angrybells)].people; vict; vict = next_vict)
      {
        next_vict = vict->next_in_room;
        if(!(vict) || IS_NPC(vict) || !(IS_MORTAL(vict))) continue;
        damage(angrybells,vict,300,TYPE_UNDEFINED,DAM_PHYSICAL);
      }
    }
  ...
  if(cmd == MSG_MOBACT && buffalo->specials.fighting && chance(47))
  {
    ...
    //cast haste on self.
      ...
      for(vict = world[CHAR_REAL_ROOM(angrybells)].people; vict; vict = next_vict)
      {
        next_vict = vict->next_in_room;
        if(!(vict) || IS_NPC(vict) || !(IS_MORTAL(vict))) continue;
        damage(angrybells,vict,800,TYPE_UNDEFINED,DAM_PHYSICAL);
      }
      break;
...
{
SHERBERT_KINGDOM_START = 14470
SHERBERT_KINGDOM_END = 14480
