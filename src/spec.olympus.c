...
int hephaestus_forge_key(OBJ *obj, CHAR *ch, int cmd, char *arg)
{
    ...
    if (ch && cmd == CMD_UNLOCK && V_ROOM(ch) == HEPHAESTUS_FORGE_ROOM)
    {
        one_argument(arg, buf);
        if (*buf && !strncmp(buf, "forge", MAX_INPUT_LENGTH))
        {
            do_unlock(ch, buf, 0);
            act("The key is consumed by magic and disapears.", 0, ch, 0, 0, TO_ROOM);
            act("The key dispears in your hand, consumed by magic.", 0, ch, 0, 0, TO_CHAR);
            extract_obj(obj);
            bReturn = TRUE;
        }
    }
...
// The Forge will reroll a single item inside
int hephaestus_forge(OBJ *obj, CHAR *ch, int cmd, char *arg)
{
    OBJ *tmp;
    CHAR *olympus_key_mob;

    int olympus_mobs[7] = {OLYMPUS_ZEUS, OLYMPUS_HERA, OLYMPUS_POSEIDON, OLYMPUS_ARES, OLYMPUS_APOLLO, OLYMPUS_ARTEMIS, OLYMPUS_HEPHAESTUS};

    // Only allow a single item in the forge
    if (cmd == CMD_PUT)
    {

        if (OBJ_CONTAINS(obj))
        {
            send_to_char("The forge rejects additional items.\r\n", ch);
            return TRUE;
        }
    }
    // On Zone Reset - Load the Key to the forge on one of the bosses.
    if (cmd == MSG_ZONE_RESET)
    {
        // No Key Found
        if (obj_proto_table[real_object(HEPHAESTUS_FORGE_KEY)].number < 1)
        {

            // Find a random Olympus Mob except for Typhon and load the key.
            olympus_key_mob = get_ch_world(olympus_mobs[number(0, NUMELEMS(olympus_mobs) - 1)]);

            if (olympus_key_mob)
            {
                tmp = read_object(HEPHAESTUS_FORGE_KEY, VIRTUAL);
                obj_to_char(tmp, olympus_key_mob);
            }
        }
    }
    return FALSE;
...
// Use hammer on the forge to reroll an item.
int hephaestus_forge_hammer(OBJ *obj, CHAR *ch, int cmd, char *arg)
{
    CHAR *owner;
    char buf[MIL];
    int forge_room;
    forge_room = real_room(HEPHAESTUS_FORGE_ROOM);
    OBJ *forge, *tmp;
    int new_obj;
    ...
    if (cmd == CMD_USE)
    {
        ...
        /* Don't spec if not in the same room as the forge. */
        if (CHAR_REAL_ROOM(owner) != forge_room)
        {
            send_to_char("You need to find a forge to use this hammer.\r\n", owner);

            return TRUE;
        }
        // Make sure the forge exists.
        forge = get_obj_by_vnum_in_room(HEPHAESTUS_FORGE, CHAR_REAL_ROOM(ch));

        if (!forge)
        {

            send_to_char("There is nothing here to hit with this hammer.\r\n", owner);
            return TRUE;
        }

        // Check if the Object has anythign in it.
        if (forge && OBJ_CONTAINS(forge))
        {
            // Get the item in the Forge - Get its ID, extract the Obj and load a new copy of the same item.
            for (OBJ *temp_obj = OBJ_CONTAINS(forge), *temp_obj_next; temp_obj; temp_obj = temp_obj_next)
            {
                temp_obj_next = OBJ_NEXT_CONTENT(temp_obj);

                // Track the ID number of the object.
                new_obj = V_OBJ(temp_obj);
                tmp = read_object(new_obj, VIRTUAL);
                obj_to_obj(tmp, forge);
                // Remove the old and replace it.
                extract_obj(temp_obj);
            }

            act("The forge glows with magical energy and then goes dark.", FALSE, owner, obj, 0, TO_ROOM);

            act("The $o shatters in $n's hands as they strike the forge!", FALSE, owner, obj, 0, TO_ROOM);
            act("The $o shatters in your hand as you strike the forge!", FALSE, owner, obj, 0, TO_CHAR);
        ...
        {
            send_to_char("There is nothing in the forge.\r\n", owner);
            return TRUE;
        }
...
int olympus_typhonroom(int room, CHAR *ch, int cmd, char *arg)
{
    ...
    if (cmd == MSG_ZONE_RESET)
    {
        ...
        {
            if (!IS_NPC(vict) || (V_MOB(vict) != OLYMPUS_TYPHON))
                continue;
    ...
    if (sealRoomsOccupied)
    {
        ...
        for (vict = world[real_room(TYPHON_ROOM)].people; vict; vict = vict->next_in_room)
        {
            if (!IS_NPC(vict) || (V_MOB(vict) != OLYMPUS_TYPHON_SEALED))
                continue;
            // If Typhon is found and the seals are occupied then we want to remove the sealed version and summon the real.
        ...
        if (mob->specials.fighting)
        {
            ...
            case 2: // Fire Phase
                ...
                case 10:
                    ...
                    if (!(main_vict) || IS_NPC(main_vict) || !(IS_MORTAL(main_vict)))
                        return FALSE;
                    ...
                    {
                        next_vict = vict->next_in_room;
                        if (!(vict) || !(main_vict) || IS_NPC(vict) || !(IS_MORTAL(vict)))
                            continue;
        ...
        if (mob->specials.fighting)
        {
            ...
            case 5: // Chain Lightning
                ...
                if (count_mortals_room(mob, TRUE) > 1)
                {
                    ...
                    if (!(vict) || IS_NPC(vict2) || !(IS_MORTAL(vict2)))
                        return FALSE;
                    ...
                    if (count_mortals_room(mob, TRUE) > 2)
                    {
                        ...
                        if (!(vict) || !(vict2) || IS_NPC(vict3) || !(IS_MORTAL(vict3)))
                            return FALSE;
...
int olympus_apollo(CHAR *mob, CHAR *ch, int cmd, char *arg)
{
    ...
    case MSG_MOBACT:
        ...
        if (!mob->specials.fighting)
        {
            ...
            {
                if (!IS_NPC(vict) || (V_MOB(vict) != OLYMPUS_ARTEMIS))
                    continue;
...
int olympus_artemis(CHAR *mob, CHAR *ch, int cmd, char *arg)
{
    ...
    case MSG_MOBACT:
        ...
        if (!mob->specials.fighting)
        {
            ...
            for (vict = world[real_room(LIBRARY_ROOM)].people; vict; vict = vict->next_in_room)
            {
                if (!IS_NPC(vict) || (V_MOB(vict) != OLYMPUS_APOLLO))
                    continue;
...
int olympus_hephaestus(CHAR *mob, CHAR *ch, int cmd, char *arg)
{
    ...
    char *hephaestus_speak[6] = {"Even gods need the tools I create.", "I craft destiny with my hammer and anvil.", "Every strike of my hammer shapes the fate of gods and mortals alike.", "Even beauty requires fire and fury to be forged.", "The forge never sleeps, nor do its creations.", "They admire my works but fear the hands that create them."};
